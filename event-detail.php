...
<head>
    ...
    <style>
    .detail-banner {
        height: 420px; position: relative; overflow: hidden; margin-top: 68px;
    }
    .detail-banner-bg {
        position: absolute; inset: 0;
        background-size: cover; background-position: center top;
        filter: blur(60px) brightness(0.18) saturate(2.5);
        transform: scale(1.15);
    }
    .detail-banner-overlay {
        position: absolute; inset: 0;
        background: linear-gradient(to bottom, rgba(0,0,0,0.1) 0%, var(--pulse-bg, #0a0a0f) 85%);
    }
    .detail-layout {
        display: grid;
    ...
    @media (max-width: 992px) {
        ...
        .ticket-sticky { position: static !important; }
    }
    .poster-wrap { margin-top: -180px; position: relative; z-index: 2; max-width: 200px; }
    .poster-inner { border-radius: 10px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.6); aspect-ratio: 2/3; }
    .poster-inner img { width: 100%; height: 100%; object-fit: cover; display: block; }
    .detail-section {
    ...
    .detail-section-title {
        ...
        color: var(--pulse-muted); margin-bottom: 16px;
    }
    ...
    .meta-icon {
        width: 34px; height: 34px; background: rgba(82,71,184,0.12); border-radius: 6px;
        display: flex; align-items: center; justify-content: center;
        color: var(--pulse-accent); flex-shrink: 0; font-size: 0.72rem; font-weight: 700;
    }
    .meta-label { font-size: 0.68rem; color: var(--pulse-muted); margin-bottom: 2px; }
    .meta-value { font-size: 0.86rem; font-weight: 500; color: var(--pulse-white); }
    .ticket-sticky { position: sticky; top: 90px; }
    .ticket-card { background: var(--pulse-surface); border: 1px solid var(--pulse-border); padding: 22px; }
    .ticket-card-title { font-size: 0.62rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--pulse-muted); margin-bottom: 18px; }
    .tier-row { display: flex; align-items: center; justify-content: space-between; padding: 11px 0; border-bottom: 1px solid var(--pulse-border); }
    .tier-row:last-of-type { border-bottom: none; }
    .tier-cat { font-size: 0.58rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--pulse-accent); margin-bottom: 2px; }
    .tier-name { font-size: 0.85rem; font-weight: 500; color: var(--pulse-white); }
    .tier-avail { font-size: 0.7rem; color: var(--pulse-muted); margin-top: 2px; }
    .tier-price { font-size: 1rem; font-weight: 700; color: var(--pulse-white); text-align: right; }
    .tier-price small { font-size: 0.65rem; color: var(--pulse-muted); font-weight: 400; display: block; }
    .btn-get-tickets {
        display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%;
        background: var(--pulse-accent); color: #fff; border: none; padding: 15px;
        font-family: var(--font-body); font-size: 0.8rem; font-weight: 600;
        letter-spacing: 0.12em; text-transform: uppercase; text-decoration: none;
        margin-top: 18px; transition: background 0.2s; cursor: pointer;
    }
    .btn-get-tickets:hover { background: #4239a6; color: #fff; text-decoration: none; }
    .btn-get-tickets.disabled { opacity: 0.4; pointer-events: none; }
...
<body>
    ...
    <div class="detail-banner">
        <div class="detail-banner-bg" style="background-image:url('<?= htmlspecialchars(resolveImageSrc($images['banner'])) ?>');"></div>
    ...
    <div class="container-fluid px-5">
        <nav style="padding:14px 0 0;">
            <ol class="breadcrumb mb-0" style="background:none;padding:0;font-size:0.78rem;">
                ...
                <li class="breadcrumb-item"><a href="events.php" style="color:var(--pulse-muted);text-decoration:none;">Events</a></li>
            </ol>
        </nav>
        ...
        <div class="detail-layout">
            <div>
                <div class="d-flex gap-4 align-items-start mb-5" style="flex-wrap:wrap;">
                    <div class="poster-wrap d-none d-md-block">
                        <div class="poster-inner">
                            <img src="<?= htmlspecialchars(resolveImageSrc($images['poster'] ?: $images['banner'])) ?>" alt="<?= htmlspecialchars($event['title']) ?>">
                        </div>
                    </div>
                    <div style="flex:1;min-width:220px;padding-top:8px;">
                        <div class="d-flex flex-wrap gap-2 mb-3">
                            <span style="background:rgba(82,71,184,0.18);color:var(--pulse-accent);border:1px solid rgba(82,71,184,0.3);font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;padding:4px 12px;">
                                <?= htmlspecialchars($event['category']) ?>
                            </span>
                            <?php if ($totalAvail > 0 && $totalAvail < 50): ?>
                            <span style="background:rgba(217,82,122,0.12);color:#D9527A;border:1px solid rgba(217,82,122,0.25);font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;padding:4px 12px;">Selling Fast</span>
                            <?php elseif ($totalAvail === 0): ?>
                            <span style="background:rgba(226,75,74,0.1);color:#e24b4a;border:1px solid rgba(226,75,74,0.25);font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;padding:4px 12px;">Sold Out</span>
                            <?php endif; ?>
                        </div>

                        <h1 style="font-size:clamp(1.4rem,3vw,2.4rem);line-height:1.15;margin-bottom:14px;color:var(--pulse-white);">
                            <?= htmlspecialchars($event['title']) ?>
                        </h1>

                        <div style="display:flex;flex-wrap:wrap;gap:18px;color:var(--pulse-muted);font-size:0.83rem;margin-bottom:20px;">
                        ...
                        <div class="d-flex gap-3 flex-wrap">
                            <?php if ($totalAvail > 0): ?>
                            <a href="booking.php?event_id=<?= $event_id ?>" class="btn btn-accent" style="display:inline-flex;align-items:center;gap:8px;padding:12px 26px;">
                                Get Tickets
                            </a>
                            <?php else: ?>
                            <button class="btn btn-accent" disabled style="opacity:0.4;display:inline-flex;align-items:center;gap:8px;padding:12px 26px;">Sold Out</button>
                            <?php endif; ?>
                            <button onclick="toggleHeart(<?= $event_id ?>)" id="heartBtn"
                                    class="btn btn-outline-accent" style="display:inline-flex;align-items:center;gap:8px;padding:12px 18px;">
                                <span id="heartText">Save</span>
                            </button>
                            <button onclick="copyLink()" class="btn btn-outline-accent" style="display:inline-flex;align-items:center;gap:8px;padding:12px 18px;">
                                Share
                            </button>
                            <?php if ($images['seatmap']): ?>
                            <button onclick="openSeatMap()" class="btn btn-outline-accent" style="display:inline-flex;align-items:center;gap:8px;padding:12px 18px;">
                                View Seat Map
                            </button>
                            <?php endif; ?>
                        </div>
                ...
                <div class="detail-section">
                    ...
                    <div class="meta-grid">
                        <div class="meta-item">
                            ...
                            <div><div class="meta-label">Date &amp; Time</div><div class="meta-value"><?= $dateStr ?> &middot; <?= $timeStr ?></div></div>
                        </div>
                        <div class="meta-item">
                            ...
                            <div>
                                ...
                                <div style="font-size:0.7rem;color:var(--pulse-muted);margin-top:2px;"><?= htmlspecialchars($event['venue_address'] ?? '') ?></div>
                            </div>
                        </div>
                        <div class="meta-item">
                            <div class="meta-icon">CT</div>
                            <div><div class="meta-label">Category</div><div class="meta-value"><?= htmlspecialchars($event['category']) ?></div></div>
                        </div>
                        <div class="meta-item">
                            <div class="meta-icon">PR</div>
                            <div><div class="meta-label">From</div><div class="meta-value"><?= $minPrice ? 'S$'.number_format($minPrice,0) : 'Free' ?></div></div>
                        </div>
                        <div class="meta-item">
                            <div class="meta-icon">AG</div>
                            <div><div class="meta-label">Age Restriction</div><div class="meta-value">All Ages Welcome</div></div>
                        </div>
                        <div class="meta-item">
                            <div class="meta-icon">DR</div>
                            <div><div class="meta-label">Doors Open</div><div class="meta-value">1 hour before show</div></div>
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-top:36px;">
                    ...
                    <h2 class="section-title" style="margin-bottom:20px;">More <em><?= htmlspecialchars($event['category']) ?></em> Events</h2>
                    <div class="related-grid">
                        <?php foreach ($related as $r): ?>
                        <a href="event-detail.php?event_id=<?= $r['event_id'] ?>" class="eb-card" style="text-decoration:none;display:flex;flex-direction:column;">
                            <div class="eb-card-img" style="height:150px;">
                                <img src="<?= htmlspecialchars(resolveImageSrc($r['banner_img'] ?? '')) ?>" alt="" loading="lazy">
            ...
            <div class="ticket-sticky">
                <div class="ticket-card">
                    ...
                    <?php if ($totalAvail === 0): ?>
                    <div style="background:rgba(226,75,74,0.1);border:1px solid rgba(226,75,74,0.3);color:#e24b4a;text-align:center;padding:10px;font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:14px;">
                        This event is sold out
                    </div>
                    <?php endif; ?>

                    <?php foreach ($sections as $i => $sec): $soldOut = $sec['avail_count'] < 1; ?>
                    <div class="tier-row">
                        <div>
                            ...
                            <div class="tier-avail">
                                ...
                                <?php elseif ($sec['avail_count'] < 20): ?>
                                    <svg width="8" height="8" viewBox="0 0 8 8" style="margin-right:5px;vertical-align:middle;flex-shrink:0;"><circle cx="4" cy="4" r="4" fill="#D9527A"/></svg><span style="color:#D9527A;">Only <?= $sec['avail_count'] ?> left</span>
                                <?php else: ?>
                                    <svg width="8" height="8" viewBox="0 0 8 8" style="margin-right:5px;vertical-align:middle;flex-shrink:0;"><circle cx="4" cy="4" r="4" fill="#0F6E56"/></svg><?= number_format($sec['avail_count']) ?> available
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <div style="background:var(--pulse-surface);border:1px solid var(--pulse-border);padding:14px 18px;margin-top:10px;">
                    <div style="font-size:0.62rem;letter-spacing:0.12em;text-transform:uppercase;color:var(--pulse-muted);margin-bottom:10px;">Share This Event</div>
                    <div style="display:flex;gap:8px;">
                        <button onclick="copyLink()" style="flex:1;background:transparent;border:1px solid var(--pulse-border);color:var(--pulse-muted);padding:8px;font-size:0.75rem;cursor:pointer;transition:all 0.2s;"
...
<?php endif; ?>
    ...
    <div id="toast" style="position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--pulse-surface);border:1px solid var(--pulse-border);padding:10px 22px;font-size:0.8rem;color:var(--pulse-white);opacity:0;transition:all 0.3s;z-index:9999;pointer-events:none;">
        Copied!
    </div>
