    ...
    <main class="container-fluid px-5 py-5">
        ...
        <form method="GET" action="manage_messages.php" class="d-flex gap-2 flex-wrap mb-4">
            <input type="text" name="q" class="form-control admin-form-control"
                   style="max-width:280px;" placeholder="Search name, email, message..."
                   value="<?= htmlspecialchars($search) ?>">

            <select name="reason" class="form-control admin-form-control" style="max-width:200px;"
                    onchange="this.form.submit()">
                <option value="">All Reasons</option>
                <?php foreach ($reasons as $r): ?>
                    <option value="<?= htmlspecialchars($r['reason']) ?>"
                            <?= $reason === $r['reason'] ? 'selected' : '' ?>>
